...
// Validação de Pontos
if (points < 0 || points > (classes.length * 10 + projects.length * 20) * 1000) {
    points = 0;
...
function setupDragAndDrop() {
    ...
    classesList.addEventListener('dragstart', (e) => {
    ...
    classesList.addEventListener('dragend', (e) => {
        const card = e.target.closest('.habit-card');
        if (card) {
    ...
    classesList.addEventListener('dragover', (e) => {
        e.preventDefault(); // Necessário para permitir drop
    });

    classesList.addEventListener('drop', (e) => {
        e.preventDefault();
        ...
        const draggedIndex = classes.findIndex(h => h.id === draggedId);
        const targetIndex = classes.findIndex(h => h.id === targetId);
        ...
        const [draggedHabit] = classes.splice(draggedIndex, 1);
        classes.splice(targetIndex, 0, draggedHabit);
...
document.getElementById('habitForm').addEventListener('submit', (e) => {
    ...
    if (id !== null) {
        ...
        const habit = classes.find(h => h.id === id);
        if (habit) {
            habit.name = name;
...
function renderDailySummary() {
    ...
    const pendingClasses = classes.map(h => {
    ...
    container.innerHTML = pendingClasses.length === 0 
        ? '<p>Todos os hábitos de hoje foram concluídos! 🎉</p>'
        : `<p>Hábitos pendentes hoje:</p><ul>${pendingClasses.map(h => `<li>${h}</li>`).join('')}</ul>`;
}
...
function deleteHabit(habitId) {
    const habit = classes.find(h => h.id === habitId);
    if (!habit) {
    ...
    if (confirm(`Tem certeza que deseja excluir o hábito "${habit.name}"?`)) {
        classes = classes.filter(h => h.id !== habitId);
        saveData();
        renderClasses();
        renderDailySummary();
...
function editHabit(id) {
    ...
    document.getElementById('classeSubmitButton').textContent = 'Salvar Alterações';
...
function resetHabitForm() {
    ...
    document.getElementById('classeSubmitButton').textContent = 'Adicionar Hábito';
...
// Persistência
function saveData() {
    console.log('Salvando dados no localStorage:', { classes, projects, points, achievements, consecutiveDays, currentDate });
