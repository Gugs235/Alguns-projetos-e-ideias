...
// Inicializar o jogo
function initGame() {
    document.querySelectorAll(".celulas").forEach(celulas => {
        celulas.addEventListener("click", handleCellClick);
        celulas.textContent = "";
    });
...
function handleCellClick(event) {
    ...
    if (board[index] === "" && gameActive) {
        ...
        updateScore();
        gameActive = false;
        } else if (board.every(celulas => celulas !== "")) {
...
function resetGame() {
    ...
    document.querySelectorAll(".celulas").forEach(celulas => {
        celulas.textContent = "";
        celulas.style.pointerEvents = "auto";
    });
    document.getElementById("gameMessage").textContent = "";
    updatePlayer();
}
